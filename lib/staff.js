...
// Who may DELETE contacts from the database. Deliberately narrower than
// "staff": deletion is irreversible, so it is limited to named individuals
// (Tammy's request, 2026-08-04: "저랑 동현님만"). Override or extend via a
// comma-separated DELETE_ALLOWED_EMAILS env var in Vercel.
const DEFAULT_DELETERS = [
  "tammy.ahn@lodestart.ai",
  "donghyun.kim@lodestart.ai",
];

export function deleteAllowlist() {
  const raw = process.env.DELETE_ALLOWED_EMAILS || "";
  const list = raw
    .split(",")
    .map((e) => e.trim().toLowerCase())
    .filter(Boolean);
  return list.length ? list : DEFAULT_DELETERS;
}

export function canDelete(email) {
  return !!email && deleteAllowlist().includes(email.toLowerCase());
}
